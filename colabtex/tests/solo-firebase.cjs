...
const {initializeApp,deleteApp}=require('firebase/app');const {getAuth,connectAuthEmulator,signInAnonymously}=require('firebase/auth');const {getDatabase,connectDatabaseEmulator,get,ref,set}=require('firebase/database');
...
(async()=>{
 ...
 console.log('Firebase: récord concurrente, desempate, lectura y aislamiento entre usuarios correctos.');for(const x of clientes)await deleteApp(x.app);clearTimeout(timeout);
