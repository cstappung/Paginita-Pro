...
class Terrain{
 ...
 crater(x,y,r){x=Math.round(x*10)/10;y=Math.round(y*10)/10;r=Math.round(r*10)/10;let removed=0;for(let yy=Math.max(0,Math.floor(y-r));yy<=Math.min(H-1,Math.ceil(y+r));yy++){const dy=yy-y;if(dy*dy>r*r)continue;const reach=Math.sqrt(r*r-dy*dy),left=Math.max(0,Math.ceil(x-reach)),right=Math.min(W-1,Math.floor(x+reach));for(let xx=left;xx<=right;xx++){let i=yy*W+xx;removed+=this.mask[i];this.mask[i]=0;}}
...
class Game{
 constructor(options={}){
  ...
  /* En línea nadie arranca solo: el turno 1 lo abre cada cliente con la
     misma semilla, y los siguientes llegan como una foto del estado. */
  if(o.online)this.phase='espera';else this.nextTurn();
 }
 /* Mismo mulberry32 que randomSeed, pero con el estado a la vista para
    poder mandarlo por la red y retomarlo en otra máquina. */
 rng(){let s=this.rngS=(this.rngS+0x6D2B79F5)|0;let t=Math.imul(s^s>>>15,1|s);t=t+Math.imul(t^t>>>7,61|t)^t;return((t^t>>>14)>>>0)/4294967296;}
 ...
 walkUnit(u,dir,dt){if(this.energy<=0||u.hp<=0)return;const speed=u.ground?98:77,goal=dir*speed;u.drive=(u.drive||0)+(goal-(u.drive||0))*(1-Math.exp(-dt*(u.ground?18:7)));if(Math.abs(u.drive)<1){u.drive=0;return;}dir=Math.sign(u.drive);const dx=u.drive*dt;let nx=clamp(u.x+dx,12,W-12),step=0;
  ...
  const distance=Math.abs(nx-u.x),before=u.walk;u.x=nx;u.y-=step;u.facing=dir;u.walk+=distance*.14;this.energy=Math.max(0,this.energy-distance);if(distance>0&&u.ground&&Math.floor(u.walk/3)!==Math.floor(before/3))this.emit('step',{x:u.x,y:u.y});
 }
 unitPhysics(u,dt){
 ...
 update(dt=STEP){
  if(this.paused||this.phase==='over'||this.phase==='espera')return;this.time+=dt;this.jumpBuffer=Math.max(0,this.jumpBuffer-dt);
  ...
  }else if(this.phase==='settle'){this.delay-=dt;this.settleElapsed+=dt;if(this.delay<=0&&(this.isSettled()||this.settleElapsed>5.5)){if(this.options.online){this.phase='espera';this.keys={left:false,right:false};this.emit('espera');}else this.nextTurn();}}
 }
 /* La foto que viaja al acabar cada turno en línea. Todo lo que el turno
    siguiente necesita y nada que nextTurn() vaya a pisar. Los agujeros
    van como delta: `hb` es cuántos había al empezar el turno y `hn` los
    nuevos, porque la lista entera crece partida a partida. */
 exportState(hb=0){const r=v=>Math.round(v*1000)/1000;return{rs:this.rngS,wa:this.water,tu:this.turn,ro:this.round,ti:this.teamIndex,tm:r(this.time),np:this.nextProjectile,
  te:this.teams.map(t=>[t.cursor,t.damage,t.kills,t.ammo.map(a=>a===Infinity?-1:a)]),
  un:this.units.map(u=>[r(u.x),r(u.y),u.hp,r(u.vx),r(u.vy),u.ground?1:0,r(u.coyote),u.facing,r(u.walk),r(u.hurt),u.eliminated?1:0,u.lastHit,r(u.landing),r(u.drive||0)]),
  cr:this.crates.map(c=>[r(c.x),r(c.y),r(c.vy),c.kind==='health'?1:0,c.alive?1:0,c.landed?1:0]),
  ba:this.barrels.map(b=>[r(b.x),r(b.y),b.alive?1:0]),
  hb,hn:this.terrain.holes.slice(hb).map(h=>[h.x,h.y,h.r])};}
 /* `holes` es la lista completa de cráteres [x,y,r]. Si la local es un
    prefijo de ella solo se tallan los que faltan; si no, el terreno se
    rehace desde cero y se devuelve true para que el dibujo lo rehaga. */
 importState(s,holes){
  this.rngS=s.rs|0;this.water=s.wa;this.turn=s.tu;this.round=s.ro;this.teamIndex=s.ti;this.time=s.tm;this.nextProjectile=s.np;
  s.te.forEach((d,i)=>{const t=this.teams[i];if(!t)return;t.cursor=d[0];t.damage=d[1];t.kills=d[2];t.ammo=d[3].map(a=>a<0?Infinity:a);});
  s.un.forEach((d,i)=>{const u=this.units[i];if(!u)return;[u.x,u.y,u.hp,u.vx,u.vy]=d;u.ground=!!d[5];u.coyote=d[6];u.facing=d[7];u.walk=d[8];u.hurt=d[9];u.eliminated=!!d[10];u.lastHit=d[11];u.landing=d[12];u.drive=d[13];});
  this.crates=s.cr.map(d=>({x:d[0],y:d[1],vy:d[2],kind:d[3]?'health':'ammo',alive:!!d[4],landed:!!d[5]}));
  this.barrels=s.ba.map(d=>({x:d[0],y:d[1],alive:!!d[2]}));
  const t=this.terrain.holes;let same=t.length<=holes.length;for(let i=0;same&&i<t.length;i++)same=t[i].x===holes[i][0]&&t[i].y===holes[i][1]&&t[i].r===holes[i][2];
  if(!same)this.terrain=new Terrain(this.options.map);
  for(let i=this.terrain.holes.length;i<holes.length;i++)this.terrain.crater(holes[i][0],holes[i][1],holes[i][2]);
  this.projectiles=[];this.keys={left:false,right:false};this.jumpBuffer=0;this.aimTarget=null;this.phase='espera';this.winner=undefined;
  return !same;
 }
